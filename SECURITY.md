...
> Project Scope: `OOMMF Micromagnetic Vector Field Simulator & LLG Engine`
...
We actively maintain and provide critical security updates for the following release lines of **Jirnyak/oommf**:
...
Security engineering in Jirnyak/oommf is guided by the following domain-specific threat vector analyses:

### 1. LLG Division by Zero Singularities
* **Description**: Excessive damping parameters producing unphysical NaN magnetization vectors.
...
### 2. Buffer Overflows in .omf Mesh Parsers
* **Description**: Parsing untrusted micromagnetic mesh input files with unvalidated cell dimensions.
* **Impact Rating**: HIGH / CRITICAL
* **Mitigation Strategy**: Strict schema validation, boundary fuzz testing, and automated static security analysis.
### 3. CUDA Multi-GPU Race Conditions
* **Description**: Unsynchronized peer-to-peer memory copies across asynchronous CUDA compute streams.
* **Impact Rating**: HIGH / CRITICAL
* **Mitigation Strategy**: Strict schema validation, boundary fuzz testing, and automated static security analysis.
...
If you discover a security flaw or exploit vector in **Jirnyak/oommf**, do **NOT** post it publicly in open issues or discussions.
