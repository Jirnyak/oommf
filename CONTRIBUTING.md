...
Thank you for your interest in contributing to **Jirnyak/oommf**. This project operates under strict technical standards: deep mathematical and domain correctness, zero-slop code, explicit typing, and zero unverified assumptions.
...
1. **LLG Numerical Stability**:  Magnetization unit vectors |m| = 1.0 must be preserved at every time step within 1e-12 tolerance.
2. **Demagnetizing Field FFT**:  3D convolution of magnetostatic dipoles must use zero-padded FFTW grids.
3. **Energy Minimization Monotonicity**:  Relaxation algorithms must monotonically decrease total magnetic free energy.
4. **Thread-Safe Vector Arrays**:  OpenMP parallel region loops must have zero data races on exchange field accumulation.
...
### 2.1 Prerequisites
* **Tech Stack**: `C++ / Tcl/Tk / OpenMP / CUDA / SIMD Vectorization`
...
2. **Regression Verification**: Ensure all existing test suites pass cleanly with `make test`.
